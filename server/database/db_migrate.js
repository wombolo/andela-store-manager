...
const Migration = {
  async migrate() {
    ...
    /* eslint-disable no-console */
      ...
      await pool.query('drop table if exists sales; drop table if exists products; drop table if exists profiles; ');


      console.log('Creating table products');
      await pool.query(`create table if not exists products (
        id serial not null constraint products_pkey primary key,
        title text not null,image text,description text,
        price numeric(11,2) not null,quantity integer,
